...
export class MenuPage {
  ...
  ionViewDidLoad() {
    ...
    if(d.day() == 0) {
      ...
      this.http.get('http://api.blackgarlic.id:7005/app/menu/weeks/'+d.format('YYYY-MM-DD')).map(res => res.json())
        .subscribe(data => {
  ...
  doRefresh(refresher) {
    ...
    this.http.get('http://api.blackgarlic.id:7005/app/menu/week/'+d.format('YYYY-MM-DD')).map(res => res.json())
      .subscribe(data => {
